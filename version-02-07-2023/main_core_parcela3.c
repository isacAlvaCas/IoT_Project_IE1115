...
void init_broker(void){
  ...
  mqtt_sn_create_sck(mqtt_sn_connection,
                     ...
                     ss(all_topics),
                     mqtt_sn_callback);
  mqtt_sn_sub("/valve_3",1);
}

unsigned short temp = 20;
unsigned short humedad = 17;
unsigned short pH = 7;
...
PROCESS_THREAD(init_system_process, ev, data) {
  ...
  while(1) {
      ...
      //printf("\nMessage sent. Topic: /data_3 Data:%s Time:%lu Energy:%d",  pub_test, time, energy_value);
      //printf("\nMessage sent. Topic: /data_3 Data:%s Time:%lu H:%d",  pub_test, time, humedad);
      mqtt_sn_pub("/data_3",pub_test,true,1);
