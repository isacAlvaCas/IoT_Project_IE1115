...
void init_broker(void){
  ...
  mqtt_sn_create_sck(mqtt_sn_connection,
                     ...
                     ss(all_topics),
                     mqtt_sn_callback);
  mqtt_sn_sub("/valve_1",1);
}

unsigned short temp = 20;
unsigned short humedad = 15;
unsigned short pH = 7;
...
PROCESS_THREAD(init_system_process, ev, data) {
  ...
  while(1) {
      ...
      energy_value = (uint32_t) ((Vc*((cpu*icpu)+(lpm*ilpm)+(tx*itx)+(rx*irx))) / (ticks));
      ...
      printf("\nMessage sent. Topic: /data_1 Data:%s Time:%lu Energy:%d",  pub_test, time, energy_value);
      //printf("\nMessage sent. Topic: /data_1 Data:%s Time:%lu Humedad:%d",  pub_test, time, humedad);
      mqtt_sn_pub("/data_1",pub_test,true,1);
